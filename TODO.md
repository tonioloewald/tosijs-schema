...
- [x] *(done, verified 2026-09-14)* `compilePattern` lives in `src/formats.ts` with a module-level cache and is imported by BOTH schema.ts and contract.ts — the duplicated compile expression is gone. with a
...
- [ ] *(unverified)* Pin `subschemas()` recursion branches with tests (tuple
  ...
  uncovered).
- [x] *(done 2026-09-14)* Type-level test added in `inference.types.ts`: `$`-prefixed and `x-*` keys are assignable to `JSONSchema`, and a typo'd `minumum` is pinned with `@ts-expect-error` so the allowlist is enforced at the type level too. Remainder of the original item
  signature on `JSONSchema` in `src/inference.types.ts`.
- [x] *(done)* COVERAGE.md regeneration is wired into `pack` via `make-coverage.ts`, so the drift gate covers it. (it was 6 months
...
- [x] *(DONE 2026-09-14)* README's per-import size rows are now MEASURED and regenerated by `make-coverage.ts` (each entry point bundled from `dist/index.js` through a one-line re-export shim, gzipped), so the drift gate covers them. Every row had drifted 15-20% low. Original note
...
- [ ] *(dx)* `smoke.ts` never exercises this release's payload. Make it habit that the
  ...
  `{ unknownPath: 'refuse' }`.
- [x] *(DONE 2026-09-14)* `smoke.ts` parses STDOUT only, filters for `.tgz`, and packs straight into the scratch dir via `--pack-destination` (no repo-root tarball, no cross-filesystem rename). Original note
...
- [x] *(KEEP-DECISION 2026-09-14 — not doing it)* The two passes count different things (all own keys vs only keys NOT declared in `properties`), so they cannot be merged without losing the max-short-circuit; it is asymptotically neutral, fires only when a schema has BOTH count constraints AND an `additionalProperties` schema, and it sits in the sampling hot path. Recorded so it stops resurfacing. Original note
