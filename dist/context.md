...
* **`agentContract(schemas, options?)`:** Maps root path → schema into `{ check, describe, affectedRoots }`. `check(path, value, proposal?)` judges the whole-root `proposal.proposed` (never walks paths itself) and returns `true | Error` (the Error message is the refusal reason; a non-string `path` is refused, never thrown). Options: `{ strict: false }` allows sampling (strict by default); `{ unknownPath: 'refuse' }` (default `'allow'`) makes a write touching NO contracted root a breach — an unrecognized value throws at construction rather than falling back to `'allow'`. **`affectedRoots(path): string[]`** (1.10.0) returns the contracted roots a write touches — at, under, or ABOVE — and is the matcher `check()` itself uses, so callers never re-derive it (the hand-rolled version misses `r[0].x`, silently). **It is a PREFIX test, not a path parser:** a path matches when it equals the root or continues it with `.` or `[`, so every spelling of the SUBTREE matches (`r.x`, `r[0].x`, `r["x"]`, `r.0.x` all match root `r`) — the gap is the ROOT's own spelling: `app["billing"].rate` misses root `app.billing`, as do a leading bracket (`["r"].x`) and a JSON Pointer. `{ unknownPath: 'refuse' }` is the spelling-independent remedy. A canonicalizing matcher was built for 1.10.0 and reverted (four passes, four more bypasses, plus a quadratic regex on the gate's least-trusted input); the real fix is a tokenizer over segment arrays, tracked in TODO.md.
* **Fail-closed invariants (do not weaken):** schemas are deep-copied at construction AND out of `describe()` (mutation cannot disarm the gate); construction validates against an ALLOWLIST — any key outside `ENFORCED_KEYWORDS` (exported beside the walk in `schema.ts`) + annotations + `x-*` throws, which catches `allOf`/`not`/`$ref`, typos, and future spec keywords alike — plus value-level holes: `format` outside `ENFORCED_FORMATS`, invalid `pattern` regexes, uncapped tuple `items`, non-primitive `const`/`enum` members, multi-type arrays, nested contracted roots (ambiguous judge), and non-boolean `strict` / unrecognized `unknownPath` (a permissive posture must be reachable only from an absent option); protocol breaches return an Error — any write touching a contracted root (at, under, or ABOVE it, incl. the empty path) without a proposal for that exact root, `proposal.root` mismatch, ancestor writes spanning several contracted roots (one proposal can't cover them — decompose), and a contracted `$predicate` with no registered evaluator. Boolean schemas are legal and enforced (`false` = refuse all, `true` = accept all); a builder is unwrapped only when it really is one (carries `validate`), so a plain schema with a stray `schema` key is refused instead of silently becoming that key's value. **The one deliberate opening:** `check()` returns bare `true` both for "valid write" and for "touches nothing I contract", so over UNCONTRACTED roots the default gate validates nothing — right for a surface that contracts a subset, wrong if you assumed whole-surface coverage. Use `affectedRoots(path)` to tell the two apart, or construct with `{ unknownPath: 'refuse' }`.
