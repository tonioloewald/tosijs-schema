...
| `{ strict: 0 }` / `{ strict: '' }` (e.g. from a parsed config) | silently builds a **sampling** gate | **throws** |

**Migration:** if construction throws naming `root.schema`, remove that key — it was never enforced. If it throws on `strict`, pass a real boolean.
...
`inferSchema` is also published as a self-contained subpath, `tosijs-schema/infer`, so it stays ~1.5 kB even where a bundler can't tree-shake the pre-bundled main entry. The other pieces share the validator core (one module), so importing `validate`, `s`, `filter`, or `diff` all land in the same 3.5–5.5 kB range — you pay for the core once, not per import. Every figure in the table is **measured at build time** and regenerated by `bun run pack`, so it cannot drift from what you actually get.
